...
if [ ! -f /var/lib/pgadmin/pgadmin4.db ]; then
    ...
    if [ ! -z "${PGADMIN_SWARM_MODE}" ] &&  [ "${PGADMIN_SWARM_MODE}" = "1" ]; then
        PGADMIN_DEFAULT_EMAIL=$(cat /run/secrets/pgadmin_default_email)
        PGADMIN_DEFAULT_PASSWORD=$(cat /run/secrets/pgadmin_default_password)
    fi
